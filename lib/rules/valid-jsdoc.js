...
module.exports = function(context) {
    ...
    var options = context.options[0] || {},
        ...
        ignoreAnonymous = options.ignoreAnonymous !== false;
    ...
    function checkJSDoc(node) {
        ...
        if (jsdocNode) {
            ...
            if (required) {
                // If the node does not have an ID, potentially ignore it
                if (!node.id && ignoreAnonymous) {
                    return;
                }
                context.report(node, "Missing JSDoc for function.");
            }
        }
